...
def request(
    base: str, path: str, *, token: str | None = None, method: str = "GET", body=None
):
    headers = {}
    data = None
    if token:
...
def main() -> int:
    ...
    parser.add_argument(
        "--mode", choices=["submit", "verify-existing"], default="submit"
    )
    ...
    if (
        result.get("worker") != "echo"
        or result.get("payload", {}).get("purpose")
        != "prove container execution and durable restart"
    ):
